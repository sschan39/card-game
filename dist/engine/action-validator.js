...
class ActionValidator {
     ...
     */
    static canMeetCondition(room, playerId, condition) {
        if (!condition)
        ...
        if (condition.zoneCheck) {
            ...
            if (check.ownedBy === 'self' || check.ownedBy === 'any') {
                if (check.zone.includes('battlefield'))
                    targetCards.push(...room.battlefield.filter(c => c.state.controllerId === playerId));
                if (check.zone.includes('graveyard'))
                    targetCards.push(...player.graveyard);
                if (check.zone.includes('hand'))
                    targetCards.push(...player.hand);
            }
            if ((check.ownedBy === 'opponent' || check.ownedBy === 'any') && opponentId) {
                const opponent = room.players[opponentId];
                if (check.zone.includes('battlefield'))
                    targetCards.push(...room.battlefield.filter(c => c.state.controllerId === opponentId));
                if (check.zone.includes('graveyard'))
                    targetCards.push(...opponent.graveyard);
                if (check.zone.includes('hand'))
                    targetCards.push(...opponent.hand);
            }
            ...
            if (targetCards.length < requiredCount)
                return false;
        }
     ...
     */
    static canPayCost(room, playerId, card, cost) {
        if (!cost)
            return true;
        const player = room.players[playerId];
        // Mana cost check
        if (cost.mana) {
            for (const [color, amount] of Object.entries(cost.mana)) {
                const playerMana = player.mana[color] ?? 0;
                if (playerMana < amount)
                    return false;
            }
        }
        if (cost.life && player.life < cost.life)
            return false;
        if (cost.tap && card.state.isTapped)
            return false;
        // Discard cost check
        if (cost.discard && player.hand.length < cost.discard)
            return false;
        return true;
    }
    /**
     ...
     */
    static canActivate(room, playerId, card, req) {
